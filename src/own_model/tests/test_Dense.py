import unittest
import numpy as np
from own_model.layers import Dense

class TestDenseLayer(unittest.TestCase):
    def setUp(self):
        self.input_size, self.output_size = 4, 3
        self.dense = Dense(self.output_size, l2_lambda=0.1)
    ...
    def test_weight_update(self):
        ...
        self.dense.forward(self.input_array, training=True)
        self.dense.backward(self.output_gradient, self.learning_rate)
        expected_new_weights = old_weights - self.learning_rate * (np.dot(self.input_array.T, self.output_gradient) + self.l2_lambda * old_weights)
