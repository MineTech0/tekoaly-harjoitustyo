...
def luo_testidata():
    ...
    tanssilaji_one_hot = to_one_hot(tanssilaji_numerot, len(label_encoder.classes_))
    ...
    # Tallennetaan koulutusdata yhteen tiedostoon
    train_data = {
        "X_train": X_train,
        "y_train": y_train,
    }
    ...
    np.savez(f"{TALLENNUS_POLKU}/kappaleet.npz", train_data, test_data)
...
def luo_validointi_data():
    ...
    def lataa_ja_muunna_audio(audio_tiedosto):
        aaltomuoto = librosa.load(audio_tiedosto, offset=KAPPALEEN_ALKU_OFFSET, duration=NÄYTTEEN_PITUUS)
        return aaltomuoto[0]

    validation_spectrograms = np.array([])

    for polku in validation_files:
        aaltomuoto = lataa_ja_muunna_audio(polku)
        spektrogrammi = luo_spektrogrammi(aaltomuoto)
        validation_spectrograms = np.append(validation_spectrograms, spektrogrammi)
        
    label_encoder = LabelEncoder()
    validation_labels_numerical = label_encoder.fit_transform(validation_labels)
    validation_labels_one_hot = to_one_hot(validation_labels_numerical, len(label_encoder.classes_) )
    
    print('Validation:', validation_spectrograms.shape, validation_labels_one_hot.shape)

    validation_data = {
        "X_val": validation_spectrograms,
        "y_val": validation_labels_one_hot,
    }
    
    np.savez(f"{TALLENNUS_POLKU}/validointi.npz", validation_data)
